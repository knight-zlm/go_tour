...
func (a Article) Update(db *gorm.DB, values interface{}) error {
	err := db.Model(&a).Update(values).Where("id = ? AND is_del = ?", a.ID, 0).Error
	if err != nil {
		return err
	}
	return nil
...
func (a Article) Get(db *gorm.DB) (*Article, error) {
	art := Article{}
	err := db.Model(&a).Where("id = ? AND state = ? AND is_del = ?", a.ID, a.State, 0).First(&art).Error
